...
/** A digital-twin message: small avatar, then a bordered bubble with a beak pointing at the avatar. */
function AssistantRow({ children }: { children: React.ReactNode }) {
  return (
    <div className="flex items-end justify-start gap-2">
      <Avatar className="h-7 w-7 shrink-0 rounded-full object-cover ring-1 ring-[var(--border)]" />
      <div className="relative max-w-[80%] rounded-2xl border border-[var(--border)] bg-[var(--panel-muted)] px-3.5 py-2 text-[var(--fg)]">
        <span
          aria-hidden
          className="absolute -left-1 bottom-2.5 h-2.5 w-2.5 rotate-45 border-b border-l border-[var(--border)] bg-[var(--panel-muted)]"
        />
        {children}
      </div>
    </div>
      ...
      {open && (
        <div className="chat-panel animate-rise mb-3 flex h-[32rem] max-h-[calc(100vh-7rem)] w-[22rem] max-w-[calc(100vw-2.5rem)] flex-col overflow-hidden">
          ...
          <div ref={scrollRef} className="flex-1 space-y-3 overflow-y-auto px-4 py-4">
            {messages.length === 0 && (
              <AssistantRow>
                <p className="text-sm leading-relaxed text-[var(--fg-muted)]">{WELCOME}</p>
              </AssistantRow>
            )}
            {messages.map((m, i) =>
              m.role === "user" ? (
